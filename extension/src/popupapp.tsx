...
import { getCourses, getData, getUser, getVersion } from "./util/data";
...
export const App = () => {
          ...
          dispatch({
            ...
            payload: {
              options: {
                ...
                hiddenCourses: res.hiddenCourses ? res.hiddenCourses : [],
              },
            },
          });
          const data = await getData(res.termID, res.schoolurl, res.capikey);
          if (chrome.action) {
            chrome.action.setBadgeText({
              text:
                data.todos.length >= 1 ? data.todos.length.toString() : "👍",
            });
            chrome.action.setBadgeBackgroundColor({
              color: badgeColor(data.todos.length),
            });
          }
          dispatch({
            type: "setData",
  ...
  console.log(state)
  console.log(state.options.hiddenCourses)
  return (
    <div className="flex flex-col space-y-4 items-center bg-red-100 h-screen justify-top">
      ...
      {page != "Inputs" && page != "TermSelect" ? (
        <IoSettings
          color={
            page == "Settings" ? "Green" : page == "Loading" ? "Grey" : "Black"
          }
      ...
      {page == "Loading" ? (
        ...
        <>
          {state.courses.length == 0 ? (
            ...
            <>
              <UserBlip user={state.user} onLogOut={handleLogOut} />
